...
class AnalyticsNotifier extends StateNotifier<AnalyticsState> {
  ...
  static AnalyticsResult runCalculations(AnalyticsInput params) {
    ...
    } else if (filterType == AnalyticsFilterType.customDate &&
        params.customRange == null) {
    ...
    for (var e in params.expenses) {
      ...
      if (e.expenseDate.isAfter(
        ...
        if (e.expenseDate.isBefore(
          ...
          equivalentPrevCatTotals[e.category] =
              (equivalentPrevCatTotals[e.category] ?? 0) + e.amount;
        }
      }
    }
    ...
    int currentElapsedDays = isPartialPeriod
        ? (params.now.difference(currentRange.start).inDays + 1)
        : currentDays;
    if (currentElapsedDays < 1) currentElapsedDays = 1;

    int prevElapsedDays = isPartialPeriod
        ? (equivalentPrevEnd.difference(prevRange.start).inDays + 1)
        : prevDays;
    ...
    final summary = ExecutiveSummary(
      totalSpend: PeriodComparison.calculate(
        currentTotalSpent,
        equivalentPrevTotalSpent,
      ),
      dailyAverage: PeriodComparison.calculate(currentDailyAvg, prevDailyAvg),
      transactionCount: PeriodComparison.calculate(
        filteredExpenses.length.toDouble(),
    ...
    for (var e in filteredExpenses) {
      final catInsight = categoryInsights.firstWhere(
        (c) => c.categoryName == e.category,
        orElse: () => CategoryInsight(
          categoryName: '',
          currentSpend: 0,
          previousSpend: 0,
          trend: PeriodComparison.calculate(0, 0),
          percentageOfTotal: 0,
          transactionCount: 1,
          averageTransaction: e.amount,
          largestTransaction: e.amount,
        ),
      );
      ...
      if (catInsight.averageTransaction > 0 &&
          e.amount > catInsight.averageTransaction * 3 &&
          e.amount > params.budgetLimit * 0.05) {
        final diffPct =
            ((e.amount - catInsight.averageTransaction) /
                catInsight.averageTransaction) *
            100;
        anomalies.add(
          AnomalyInsight(
            transaction: e,
    ...
    if (consistency.meanDailySpend > 0) {
      for (var entry in currentDailySum.entries) {
        if (entry.value > consistency.meanDailySpend * 2 &&
            entry.value > params.budgetLimit * 0.05) {
          // find top category for this day
          final dayExpenses = filteredExpenses.where(
            (e) => DateUtils.dateOnly(e.expenseDate) == entry.key,
          );
          final dCatTotals = <String, double>{};
          for (var e in dayExpenses)
            dCatTotals[e.category] = (dCatTotals[e.category] ?? 0) + e.amount;
          String topCat = dCatTotals.isEmpty
              ? 'Unknown'
              : dCatTotals.entries
                    .reduce((a, b) => a.value > b.value ? a : b)
                    .key;

          highSpendingDays.add(
            HighSpendingDayInsight(
              date: entry.key,
              amount: entry.value,
    ...
    for (var cat in categoryInsights) {
      if (cat.trend.direction == TrendDirection.increase &&
          cat.trend.percentageChange > 50.0 &&
          cat.percentageOfTotal > 10.0) {
    ...
    insightFacts.add(
      InsightFact(
        ...
        importance: velocity.status == VelocityStatus.veryFast
            ? FactImportance.critical
            : FactImportance.medium,
        context:
            'Current spent: ${currentTotalSpent}, Budget Limit: ${params.budgetLimit}',
      ),
    );
    ...
    for (var anomaly in anomalies) {
      insightFacts.add(
        InsightFact(
          ...
          formattedValue:
              '${anomaly.transaction.category} expense of ${anomaly.transaction.amount}',
    ...
    if (primaryIncrease != 'None') {
      insightFacts.add(
        InsightFact(
          ...
          context:
              'Increased by ${maxInc} compared to equivalent previous period.',
        ),
      );
    }
